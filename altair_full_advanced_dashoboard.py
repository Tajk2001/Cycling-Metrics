...
class CyclingAnalyzer:
    ...
    def _print_performance_analysis(self):
        ...
        if self.df is not None and 'heart_rate' in self.df.columns:
            ...
            print(f"Heart Rate Reserve: {hr_reserve:.1f}% (Max HR: {self.metrics.get('hr', {}).get('max', 0):.0f}bpm)")
    ...
    def print_comprehensive_metrics_table(self):
        ...
        print(f"{'Metric':<30} {'Value':<20} {'Unit':<15} {'Notes':<15}")
        print("-" * 80)
        if self.df is not None and 'heart_rate' in self.df.columns and hasattr(self, 'avg_power') and self.avg_power is not None and self.metrics.get('hr', {}).get('avg', 0) > 0:
            power_hr_ratio = self.avg_power / self.metrics.get('hr', {}).get('avg', 0)
            print(f"{'Power/HR Ratio':<30} {power_hr_ratio:<20.2f} {'W/bpm':<15} {'Efficiency':<15}")
        if self.df is not None and 'cadence' in self.df.columns and hasattr(self, 'avg_power') and self.avg_power is not None and self.metrics.get('cadence', {}).get('avg', 0) > 0:
            power_cadence_ratio = self.avg_power / self.metrics.get('cadence', {}).get('avg', 0)
        ...
        if hasattr(self, 'data_quality_report'):
            ...
            print(f"{'Metric':<30} {'Value':<20} {'Unit':<15} {'Notes':<15}")
            print("-" * 80)
            print(f"{'Data Quality Score':<30} {self.data_quality_report.get('data_quality_score', 0):<20.1f} {'/100':<15} {'':<15}")
            print(f"{'Interpolated Data':<30} {self.data_quality_report.get('interpolated_pct', 0):<20.1f} {'%':<15} {'':<15}")
            print(f"{'Outliers Removed':<30} {self.data_quality_report.get('outliers_removed', 0):<20.0f} {'':<15} {'':<15}")
...
def main():
    ...
    parser.add_argument('--save-figures', action='store_true', help='Save figures to disk')
    parser.add_argument('--fit-file', type=str, required=True, help='Path to FIT file')
    ...
    analyzer = CyclingAnalyzer(
        ...
        save_figures=args.save_figures,  # Enable figure saving
        save_dir="figures"  # Directory for saving figures
    )
    file_path = args.fit_file
    ...
    if analyzer.load_fit_file(file_path):
        ...
        # Compose advanced row (add more fields as needed)
        advanced_row = {
            "ride_id"      : file_path.split("/")[-1].replace(".fit", ""),
        ...
        if 'date' in hist.columns and pd.Series(hist['date']).notna().any():
            # Convert all date values to pandas datetime for consistent sorting
            hist['date'] = pd.to_datetime(hist['date'], errors='coerce')
            hist = pd.DataFrame(hist).sort_values(by="date")
        hist.to_csv(hist_path, index=False)
        print(f"\U0001F4C1 Advanced metrics archived to {hist_path} ({len(hist)} rides).")
        # ---------- END ARCHIVE BLOCK ----------
